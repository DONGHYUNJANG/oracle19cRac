...
### 시간 동기화 중지 * 실무에선 켜놓는다고 한다. (case by case)
systemctl stop chronyd
systemctl disable chronyd
mv /etc/chrony.conf /etc/chrony.conf.bak

### NTP 중지 * 실무에선 켜놓는다고 한다. (case by case)
...
domain-needed      # 도메인이 없는 짧은 이름은 외부로 보내지 않음
bogus-priv         # 가짜 역방향 질의 방지

<img width="635" height="311" alt="image" src="https://github.com/user-attachments/assets/d8e1f89c-bd4a-470c-83c2-1e41204e2554" />
...
권장 사항: 항상 /dev/shm은 물리 RAM의 70~80% 이내에서 설정하는 것이 정신 건강에 이롭습니다. 6Gi 서버라면 아무리 많이 잡아도 4Gi~4.5Gi 정도가 마지노선입니다.
...
/etc/sysctl.conf의 shmmax, shmall 값을 메모리값의 절반보다 큰 4GB= 4294967296(바이트) 로 수정하고 값을 확인한다.
...
shmmax = 단일 프로세스가 공유 메모리를 호출하기 위한 최대 값 (byte)

shmall = 모든 프로세스가 사용할 수 있는 총 공유 메모리 값 (페이지)
